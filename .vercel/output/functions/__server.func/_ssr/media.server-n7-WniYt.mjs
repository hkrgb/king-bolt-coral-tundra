...
//#endregion
//#region node_modules/.nitro/vite/services/ssr/assets/media.server-n7-WniYt.js
var media_server_n7_WniYt_exports = /* @__PURE__ */ __exportAll({
	C: () => UnauthorizedError,
	D: () => GROK_PROVIDERS,
	E: () => isolation_server_exports,
	S: () => isAllowedEmail,
	T: () => auth,
	_: () => shareUrl,
	a: () => serveMediaResponse,
	b: () => requireWorkshopFromRequest,
	c: () => saveGithubToken,
	d: () => githubRepoUrl,
	f: () => ACCEPT_ATTR,
	g: () => isMediaFilename,
	...
	m: () => fileUrl,
	n: () => getMediaMeta,
	o: () => githubIsConfigured,
	p: () => MAX_FILE_BYTES,
	r: () => listMedia,
	s: () => githubTokenFromEnv,
	t: () => deleteMedia,
	u: () => GITHUB_REPO,
	v: () => ForbiddenError,
	w: () => verify_server_exports,
	x: () => ALLOWED_EMAIL,
	y: () => assertWorkshopUser
});
...
var isolation_server_exports = /* @__PURE__ */ __exportAll$1({
...
async function createPgliteSql() {
	...
	const migrate = async () => {
		const migrations = /* #__PURE__ */ Object.assign({
			...
			"/migrations/0002_media.sql": _0002_media_default,
			"/migrations/0003_github.sql": _0003_github_default
		});
...
var databaseConfigured$1 = Boolean(process.env.DATABASE_URL?.trim());
if (databaseConfigured$1 && !authConfigured) console.error("[auth] DATABASE_URL is set but auth is disabled (VITE_AUTH_ENABLED=false) — requireUserId() will reject every request (fail closed) rather than share one dev user on a real database.");
...
async function requireUserId(bearerToken) {
	if (!authConfigured) {
		if (databaseConfigured$1) throw new Error("Auth is disabled (VITE_AUTH_ENABLED=false) but DATABASE_URL is set — refusing to fall back to the shared dev user against a real database.");
		return DEV_USER_ID;
	}
...
var ALLOWED_EMAIL = "info@rgb-workshop.com";
...
function bearerFromRequest() {
	...
	return header.slice(7).trim() || void 0;
}
/**
* Auth + allowlist for API routes (upload). Session from cookie or bearer.
*/
async function requireWorkshopFromRequest() {
	...
	if (!user) throw new UnauthorizedError();
	if (!isAllowedEmail(user.email)) throw new ForbiddenError("This workspace is private.");
	return {
		id: user.id,
		email: user.email
	};
...
*/
async function assertWorkshopUser(userId) {
	if (!authConfigured) {
		if (databaseConfigured()) throw new UnauthorizedError();
		if (userId !== "dev-user") throw new UnauthorizedError();
		return;
	}
	const { getSql } = await Promise.resolve().then(() => db_exports);
	if (!isAllowedEmail((await (await getSql())`
    select email from "user" where id = ${userId} limit 1
  `)[0]?.email)) throw new ForbiddenError("This workspace is private.");
}
var MAX_FILE_BYTES = 12582912;
...
var ACCEPT_ATTR = "image/jpeg,image/png,image/gif,image/webp,image/svg+xml,video/mp4,audio/mpeg,.jpg,.jpeg,.png,.gif,.webp,.svg,.mp4,.mp3";
var EXT_MIME = {
	jpg: "image/jpeg",
	jpeg: "image/jpeg",
	png: "image/png",
...
function githubCdnUrl(slug, commitSha) {
	return `https://cdn.jsdelivr.net/gh/${GITHUB_OWNER}/${GITHUB_REPO}@${commitSha && /^[a-f0-9]{7,40}$/i.test(commitSha) ? commitSha : GITHUB_BRANCH}/${slug}`;
}
/** Direct file URL on GitHub itself — available immediately after commit. */
function githubRawUrl(slug) {
	return `https://raw.githubusercontent.com/${GITHUB_OWNER}/${GITHUB_REPO}/${GITHUB_BRANCH}/${slug}`;
}
var TOKEN_KEY = "github_token";
function envToken() {
	return ((typeof process !== "undefined" ? process.env.GITHUB_TOKEN : void 0) ?? (typeof process !== "undefined" ? process.env.GITHUB_MEDIA_TOKEN : void 0) ?? (typeof process !== "undefined" ? process.env.GH_TOKEN : void 0))?.trim() || void 0;
}
...
async function assertTokenCanWrite(token) {
	const res = await fetch(`https://api.github.com/repos/${GITHUB_OWNER}/${GITHUB_REPO}`, { headers: {
		Authorization: `Bearer ${token}`,
		Accept: "application/vnd.github+json",
		"X-GitHub-Api-Version": "2022-11-28"
	} });
	if (res.status === 401 || res.status === 403) throw new Error("GitHub token 無效或沒有這個 repo 的權限");
	if (!res.ok) throw new Error(`無法連接 GitHub（${res.status}）`);
}
...
async function putGithubFile(opts) {
	const token = await getStoredGithubToken();
	if (!token) throw new Error("GitHub token is not configured");
	const path = opts.slug;
	const res = await fetch(`https://api.github.com/repos/${GITHUB_OWNER}/${GITHUB_REPO}/contents/${encodeURIComponent(path)}`, {
		method: "PUT",
		headers: {
			Authorization: `Bearer ${token}`,
			Accept: "application/vnd.github+json",
	...
	if (!res.ok) {
		...
		if (res.status === 401 || res.status === 403) throw new Error("GitHub token 無效或權限不足（需要 Contents 寫入）");
...
async function deleteGithubFile(path) {
	const token = await getStoredGithubToken();
	if (!token) return;
	const meta = await fetch(`https://api.github.com/repos/${GITHUB_OWNER}/${GITHUB_REPO}/contents/${encodeURIComponent(path)}?ref=${GITHUB_BRANCH}`, { headers: {
		Authorization: `Bearer ${token}`,
		Accept: "application/vnd.github+json",
		"X-GitHub-Api-Version": "2022-11-28"
	} });
	...
	if (!body.sha) return;
	await fetch(`https://api.github.com/repos/${GITHUB_OWNER}/${GITHUB_REPO}/contents/${encodeURIComponent(path)}`, {
		method: "DELETE",
		headers: {
			Authorization: `Bearer ${token}`,
			Accept: "application/vnd.github+json",
...
async function getMediaMeta(slug) {
...
async function getMediaFile(slug) {
...
async function serveMediaResponse(slug, request) {
	const found = await getMediaFile(slug);
	if (!found) return new Response("Not found", { status: 404 });
	const { item, bytes } = found;
	if (!bytes) {
		if (item.publicUrl) return Response.redirect(item.publicUrl, 302);
...
async function saveMediaFile(opts) {
	const mime = mimeFromName(opts.filename, opts.mimeType);
	if (!ALLOWED_MIME.has(mime) && mime !== "audio/mpeg") throw Object.assign(/* @__PURE__ */ new Error("Unsupported file type"), { status: 415 });
	const kind = kindFromMime(mime);
	if (!kind) throw Object.assign(/* @__PURE__ */ new Error("Unsupported file type"), { status: 415 });
	if (opts.bytes.byteLength === 0) throw Object.assign(/* @__PURE__ */ new Error("Empty file"), { status: 400 });
	if (opts.bytes.byteLength > 12582912) throw Object.assign(/* @__PURE__ */ new Error("File is too large (max 12 MB)"), { status: 413 });
	const sql = await getSql();
	const id = randomUUID();
	...
	for (let attempt = 0; attempt < 6; attempt += 1) {
      ...
      `;
			...
			if ((err instanceof Error ? err.message : String(err)).toLowerCase().includes("unique") && attempt < 5) continue;
			throw err;
		}
	}
...
//#endregion
export { saveGithubToken as C, GENERIC_OAUTH_ERROR_CODES as D, shareUrl as E, PACKAGE_VERSION as O, requireWorkshopFromRequest as S, serveMediaResponse as T, isAllowedEmail as _, GITHUB_REPO as a, listMedia as b, UnauthorizedError as c, deleteMedia as d, fileUrl as f, githubTokenFromEnv as g, githubRepoUrl as h, GITHUB_OWNER as i, getBaseURL as k, assertWorkshopUser as l, githubIsConfigured as m, ALLOWED_EMAIL as n, GROK_PROVIDERS as o, getMediaMeta as p, ForbiddenError as r, MAX_FILE_BYTES as s, ACCEPT_ATTR as t, auth as u, isAllowedFile as v, saveMediaFile as w, media_server_n7_WniYt_exports as x, isMediaFilename as y };
