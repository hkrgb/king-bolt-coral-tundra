...
//#region node_modules/.nitro/vite/services/ssr/assets/middleware-mziLagb2.js
/**
...
var authMiddleware = createMiddleware({ type: "function" }).client(async ({ next }) => {
	const { getBearerToken } = await import("./client-DhSpiK8Y.mjs").then((n) => n.n);
	return next({ sendContext: { bearerToken: getBearerToken() ?? void 0 } });
}).server(async ({ next, context }) => {
	const { assertSameSiteRequest } = await import("./media.server-n7-WniYt.mjs").then((n) => n.x).then((n) => n.E);
	const { requireUserId } = await import("./media.server-n7-WniYt.mjs").then((n) => n.x).then((n) => n.w);
